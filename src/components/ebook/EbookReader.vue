<template>
  <div>
    <input v-if="!book" type="file" id="input" style="font-size: 20px"/>
...
export default {
  ...
  methods: {
    ...
    initEvent() {
      ...
      this.rendition.hooks.content.register(function (contents) {
        ...
        contents.document
          .querySelectorAll(".duokan-image-single img")
          .forEach((node) => {
      ...
      this.rendition.on("mouseup", (event) => {
        ...
        if (
          ...
          event.target.parentNode.localName === "a"
        )
          return;
        const path = event.path || event.composedPath();
        if (event.target.localName === "img" && path) {
          const classList = [].concat(
            ...path.map((item) => [].concat.apply([], item.classList))
          );
          if (
            classList.findIndex((item) => item === "duokan-image-single") !== -1
          )
            return;
        }
        if (time < 200) {
          if (event.screenX - window.screenX > window.innerWidth * 0.75)
            this.nextPage();
          else if (event.screenX - window.screenX < window.innerWidth * 0.25)
            this.prevPage();
          else if (
            event.y < window.innerHeight * 0.75 &&
            event.y > window.innerHeight * 0.25
          )
            this.show();
        }
      });
  ...
  mounted() {
    ...
    // this.initEpub(new Epub("/" + fileName + ".epub"));
    inputEvent()
  },
  name: "EbookReader",
};
